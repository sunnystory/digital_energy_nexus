...
Rev. 3 note - dual role of pop_2km / pop_10km:
    Besides feeding the MVI (continuous, priority-score input), the RAW
    pop_2km and pop_10km values are the indicators tested against Stage 2's
    Tier 2 demand floors (E4 new mini-grid: pop_2km; C3 new tower: pop_10km).
    Stage 2b calibrates those floors by computing the SAME buffered
    population sums around existing mini-grid/tower sites, so R_COMMUNITY_KM
    and R_CATCHMENT_KM below MUST stay equal to Stage 2b's radii - if you
    change one, change both and re-run the calibration.

Run order: Stage 1 -> THIS SCRIPT -> Stage 2b (demand-floor calibration)
           -> Stage 2 (delivery matching).
...
# --- Buffer radii (km) ---
# R_COMMUNITY_KM and R_CATCHMENT_KM are COUPLED to Stage 2b's calibration
# buffers (R_MINIGRID_SERVICE_KM=2, R_TOWER_CATCHMENT_KM=10): the demand
# floors are percentiles of the same buffered population sums measured
# around existing infrastructure, so the radii must match on both sides.
R_COMMUNITY_KM = 2    # near-field demand / building / no-road buffer; = mini-grid LV service area (Stage 2b)
R_CATCHMENT_KM = 10   # wider demand catchment (population, RWI, bundling); = tower coverage catchment (Stage 2b)
# Road-isolation flags: 2 km = the RAI / SDG 9.1.1 personal-access standard
# (population within 2 km of an all-season road, ~20-25 min walk); 5 km =
# severe isolation, reused by Stage 2 as the C3/C4 buildability screen
# (one band beyond RAI; = OnSSET's most favorable distance-to-roads class).
R_NEAR_ROAD_KM = 2
R_FAR_ROAD_KM = 5
